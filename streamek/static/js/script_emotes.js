...
function addGuessedStreamerToList(streamerName, imageURL, isCorrectGuess = false) {
    ...
    streamerImage.src = imageURL || '../images/default-image-url.png'; // Fallback image
...
document.addEventListener('DOMContentLoaded', function () {
        // Check game status when the page is loaded
        fetch('/api/game-status')
            .then(response => response.json())
            .then(data => {
                if (data.game_started) {
                    startGame(); // Call the startGame function if the game has started
                }
            })
    ...
    function startGame() {
        ...
        fetch('/api/reset-game-flag', { method: 'POST' })
        .then(response => response.json())
        .then(data => console.log('Game flag reset:', data))
        .catch(error => console.error('Error resetting game flag:', error));
    }
...
function renderEmojis() {
    ...
    for (let i = 0; i < emojisData.length; i++) {
        ...
        if (i < currentEmojiIndex) {
            ...
            placeholderImg.src = 'static/images/q2.png';
...
async function checkGuess() {
    ...
    try {
        ...
        let firstTry = attemptsCount === 0; // Determine if it's the first try
        if (streamerNames.includes(guess)) {
            ...
            if (guess === correctStreamer) {
                ...
                // Update user statistics for a correct guess
                await updateUserStats(attemptsCount, true, firstTry);
            } else {
                ...
                // Update user statistics for an incorrect guess
                await updateUserStats(attemptsCount, false, firstTry);
            }
...
async function updateUserStats(attempts, won, firstTry) {
    try {
        const response = await fetch('/update_user_stats', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ attempts, won, first_try: firstTry }),
        });
